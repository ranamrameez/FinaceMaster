...
const TEST_CATEGORIES: Category[] = [
  ...
  { id: 'cat_property_tax', serialNumber: 2, name: 'Property tax' },
  { id: 'cat_repairs', serialNumber: 3, name: 'Repairs' },
];
...
describe('propertyNetIncome', () => {
  it('nets rent income minus expenses for one property', () => {
    ...
    const entries = [
      entry({ id: 'e1', propertyId: 'p1', isDeposit: true, amount: 1500 }),
      entry({ id: 'e2', propertyId: 'p1', isDeposit: false, amount: 300, categoryID: 'cat_maintenance' }),
    ];
  ...
  it('ignores entries for other properties', () => {
    ...
    const entries = [
      ...
      entry({ id: 'e2', propertyId: 'other', isDeposit: true, amount: 99999 }),
    ];
...
describe('netIncomeByCurrency', () => {
  it('groups multiple properties by currency without converting', () => {
    ...
    const entries = [
      ...
      entry({ propertyId: 'p2', isDeposit: true, amount: 500 }),
      entry({ propertyId: 'p3', isDeposit: true, amount: 2000 }),
    ];
...
describe('netIncomeByProperty', () => {
  it('returns one row per property in the given currency, others excluded', () => {
    ...
    const entries = [
      entry({ propertyId: 'p1', isDeposit: true, amount: 1000 }),
      entry({ propertyId: 'p2', isDeposit: true, amount: 500 }),
      entry({ propertyId: 'p2', isDeposit: false, amount: 100, categoryID: 'cat_repairs' }),
      entry({ propertyId: 'p3', isDeposit: true, amount: 9999 }),
    ];
...
describe('propertyByCategory', () => {
  it('buckets rent income separately from categorized expenses', () => {
    const p = property({ id: 'p1' });
    const entries = [
      entry({ id: 'e1', isDeposit: true, amount: 1500 }),
      entry({ id: 'e2', isDeposit: false, amount: 200, categoryID: 'cat_maintenance' }),
      entry({ id: 'e3', isDeposit: false, amount: 100, categoryID: 'cat_maintenance' }),
      entry({ id: 'e4', isDeposit: false, amount: 50, categoryID: 'cat_property_tax' }),
    ];
    const byCategory = propertyByCategory(p, entries, TEST_CATEGORIES);
...
describe('propertyMonthlyRollup', () => {
  it('groups income/expense/net by month, chronologically', () => {
    const p = property({ id: 'p1' });
    const entries = [
      entry({ id: 'e1', date: '2026-02-05', isDeposit: true, amount: 1000 }),
      entry({ id: 'e2', date: '2026-01-10', isDeposit: true, amount: 1000 }),
      entry({ id: 'e3', date: '2026-01-15', isDeposit: false, amount: 200, categoryID: 'cat_repairs' }),
    ];
