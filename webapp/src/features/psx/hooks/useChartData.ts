...
import { usePSXDerived } from './usePSXDerived';

function monthBucket(dateStr: string) {
  return dateStr.slice(0, 7);
}

/** PSX's equivalent of the QSE useChartData hook — same aggregations, PSX
 * store + fee calculator. Kept as a straight port so both modules' charts
 * stay in sync if the aggregation logic changes; genuinely shared logic
 * (there's no PSX-specific twist here) but duplicated rather than factored
 * out to match the rest of this module's file-per-exchange structure. */
export function useChartData() {
  const { workbook, positions, rows, calcFee } = usePSXDerived();
  ...
  const feesByMonth = useMemo(() => {
    ...
    workbook.transactions.forEach((tx) => {
      const m = monthBucket(tx.date);
      map[m] = (map[m] || 0) + calcFee(tx.shares * tx.price, tx.action === 'BUY', { shares: tx.shares, tx });
    });
